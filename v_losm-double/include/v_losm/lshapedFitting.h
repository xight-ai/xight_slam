...
public:
    ...
    cv::RotatedRect FitBox(std::vector<cv::Point2f>* pointcloud_ptr)
    {
        ...
        if (points.size() < 3){
            cout << "Too few input points." << endl;
    ...
    cv::RotatedRect FitBox(std::vector<cv::Point2f>* pointcloud_ptr_min, std::vector<cv::Point2f>* pointcloud_ptr_max)
    {
        std::vector<cv::Point2f>& points_min = *pointcloud_ptr_min;
        std::vector<cv::Point2f>& points_max = *pointcloud_ptr_max;

        if (points_min.size() < 3){
            cout << "Too few input points." << endl;
            return cv::RotatedRect();
        }

        // Initialize Contour Points Matrix.
        cv::Mat Matrix_pts = cv::Mat::zeros(points_min.size(), 2, CV_64FC1);
        cv::Mat Matrix_bound = cv::Mat::zeros(points_min.size()+points_max.size(), 2, CV_64FC1);

        for (size_t i = 0; i < points_min.size(); ++i)
        {
            Matrix_pts.at<double>(i, 0) = points_min[i].x;
            Matrix_pts.at<double>(i, 1) = points_min[i].y;
            Matrix_bound.at<double>(i, 0) = points_min[i].x;
            Matrix_bound.at<double>(i, 1) = points_min[i].y;
        }

        for(size_t i=0; i < points_max.size(); ++i)
        {
            Matrix_bound.at<double>(points_min.size()+i, 0) = points_max[i].x;
            Matrix_bound.at<double>(points_min.size()+i, 1) = points_max[i].y;
        }
        ...
        cv::Mat c1_s = Matrix_bound * e1_s.t();
        cv::Mat c2_s = Matrix_bound * e2_s.t();

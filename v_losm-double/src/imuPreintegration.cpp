...
public:
    ...
    IMUPreintegration()
    {
        ...
        subOdometry = nh.subscribe<nav_msgs::Odometry>("v_losm/Optimization/odometry", 10, &IMUPreintegration::odometryHandler, this, ros::TransportHints().tcpNoDelay());
        // 发布imu里程计
        pubImuOdometry = nh.advertise<nav_msgs::Odometry>("/imuOdometry", 2000);
    ...
    void odometryHandler(const nav_msgs::Odometry::ConstPtr &odomMsg)
    {
        ...
        // std::cout << "currentCorrectionTime: " << fixed << currentCorrectionTime << std::endl;
        ...
        if(systemInitialized == false){
            ...
            while (!imuQueOpt.empty())
            {
                if(ROS_TIME(&imuQueOpt.front()) < currentCorrectionTime - delta_t){
                    // std::cout << "imuQueOptTime: " << fixed << ROS_TIME(&imuQueOpt.front()) << std::endl;
            ...
            // 清空因子图。因子已经被记录到优化器中。这是在gtsam的官方example的递增式优化流程中的用法
        ...
        if(key == 100){
            ...
            // 重置优化器和因子图
            resetOptimization();
            // 添加位姿先验因子，把上一次优化出的位姿作为重新初始化的priorPose
            gtsam::PriorFactor<gtsam::Pose3> priorPose(X(0), prevPose_, updatedPoseNoise);
            graphFactors.add(priorPose);

            // 添加速度先验因子, 把上一次优化出的速度作为重新初始化的priorVel
            gtsam::PriorFactor<gtsam::Vector3> priorVel(V(0), prevVel_, updatedVelNoise);
            graphFactors.add(priorVel);

            // 添加偏置先验因子, 把上一次优化出的bias作为重新初始化的priorBias
            gtsam::PriorFactor<gtsam::imuBias::ConstantBias> priorBias(B(0), prevBias_, updatedBiasNoise);
            graphFactors.add(priorBias);

            // 变量节点赋初值，将prior状态设置成初始估计值
            graphValues.insert(X(0), prevPose_);
            graphValues.insert(V(0), prevVel_);
            graphValues.insert(B(0), prevBias_);
        ...
        if(!imuQueImu.empty()){
            ...
            for (int i = 0; i < (int)imuQueImu.size(); ++i){
                ...
                double imuTime = ROS_TIME(thisImu);
                double dt = (lastImuQT < 0) ? (1.0 / 400):(imuTime - lastImuQT);
    ...
    void imuHandler(const sensor_msgs::Imu::ConstPtr &imu_raw)
    {
        ...
        odometry.header.frame_id = odometryFrame; // pose所在坐标系，这里已经对齐到odom坐标系
        odometry.child_frame_id = "odom_imu";     // twist所在坐标系，这里是IMU坐标系
        // 将IMU里程计完全对齐到雷达（一个平移关系）
